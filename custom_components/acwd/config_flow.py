...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    client = ACWDClient(data[CONF_USERNAME], data[CONF_PASSWORD])
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        _LOGGER.info("ACWD Config Flow: Displaying user setup form")
        ...
        if user_input is not None:
            try:
                ...
                # Set unique ID based on account number
                await self.async_set_unique_id(str(info["account_number"]))
